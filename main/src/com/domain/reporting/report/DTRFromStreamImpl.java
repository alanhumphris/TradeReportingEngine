 ...
 */
public class DTRFromStreamImpl implements DailyTradeReport {
    ...
    public List<ReportLine> dailySettlementIncomingSummary(List<InstructionProcessed> instructions) {
        ...
        Map<LocalDate, Double> sumByDate = generateDailySummary(instructions, EnumTradeDirection.INCOMING);

        if ((sumByDate != null) && (sumByDate.size() > 0)) {
            List<ReportLine> tempResult = sumByDate.entrySet().stream().sorted(Comparator.comparing(e -> e.getKey()))
    ...
    public List<ReportLine> dailySettlementOutgoingSummary(List<InstructionProcessed> instructions) {
        ...
        Map<LocalDate, Double> sumByDate = generateDailySummary(instructions, EnumTradeDirection.OUTGOING);
    ...
    public List<ReportLine> entityRanking(List<InstructionProcessed> instructions) {
        ...
        // outgoing section
        Map<String, Optional<InstructionProcessed>> outgoingMaxByEntity
        ...
        if ((outgoingMaxByEntity != null) && (outgoingMaxByEntity.size() > 0)) {
            List<ReportLine> tempResult = outgoingMaxByEntity.entrySet().stream().sorted(Comparator.comparing(e -> e.getValue().get().getTradeValueUSD(), Comparator.reverseOrder()))
        ...
        // incoming section
        Map<String, Optional<InstructionProcessed>> incomingMaxByEntity
        ...
        if ((incomingMaxByEntity != null) && (incomingMaxByEntity.size() > 0)) {
            List<ReportLine> tempResult = incomingMaxByEntity.entrySet().stream().sorted(Comparator.comparing(e -> e.getValue().get().getTradeValueUSD(), Comparator.reverseOrder()))
                    .map(e -> new EntityRankingReportLine(e.getKey(), e.getValue().get().getTradeValueUSD(), EnumCurrencyInfo.US_DOLLAR)).collect(Collectors.toList());
     ...
     * @return
     */
    protected Map<LocalDate, Double> generateDailySummary(List<InstructionProcessed> instructions, EnumTradeDirection tradeDirection) {
     ...
     * @return
     */
    protected Map<String, Optional<InstructionProcessed>> generateEntityRanking(List<InstructionProcessed> instructions, EnumTradeDirection tradeDirection) {
        if  ((instructions == null) || (tradeDirection == null)) {
            return null;
        }
        
        Map<String, Optional<InstructionProcessed>> result
                = instructions.stream()
                        .filter(e -> e.getTradeDirection() == tradeDirection)
                        .collect(
                                Collectors.groupingBy(InstructionProcessed::getEntityName,
